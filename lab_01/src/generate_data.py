...
def gen_order() -> None:
    created_at = _random_date(datetime(2015, 1, 1, 0, 0), datetime(2021, 9, 10, 0, 0))
    ...
    order = Order(
        ...
        dst_address=choice(CUSTOMER_ADDRESSES).id,
        src_address=choice(REST_ADDRESSES).id,
    ...
    ORDERS.append(order)
    gen_order_details(order_id, restaurant.id)
